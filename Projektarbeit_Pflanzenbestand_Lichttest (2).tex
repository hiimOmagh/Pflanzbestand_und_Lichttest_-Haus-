...
In diesem Kapitel werden die wissenschaftlichen und technischen Grundlagen beschrieben, die der App zugrunde liegen:
\begin{itemize}
    \item Grundlagen der Pflanzenphysiologie: Photosynthetisch aktive Strahlung (PAR), PPFD (Photosynthetic Photon Flux Density) und Tageslichtintegral (DLI).
    \item Nutzung von Smartphone-Sensoren (Lichtsensor, Kamera) für Messungen.
    \item Relevanz von Temperatur, Luftfeuchtigkeit und Standortorientierung für Zimmerpflanzen.
    \item Mobile-Computing-Aspekte: Datenerfassung, lokale Persistenz mit Room, Export und Benutzerfreundlichkeit.
\end{itemize}
