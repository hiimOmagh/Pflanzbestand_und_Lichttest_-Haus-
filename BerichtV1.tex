...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=0.85\textwidth]{docs/dashboard_uebersicht.png}
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=0.85\textwidth]{docs/lichttest_dialog.png}
...
\begin{tikzpicture}[
    ...
    actor/.style={rectangle, draw=black, thick, rounded corners, fill=green!15, align=center, minimum width=1.9cm, minimum height=0.9cm},
    usecase/.style={ellipse, draw=black, thick, fill=blue!10, minimum width=3.8cm, minimum height=1.4cm, align=center}
]

\node[actor] (gardener) {Gärtner*in\\(Primärnutzer)};
\node[usecase, right=of gardener] (manage) {Pflanzenbestand pflegen};
\node[usecase, below=of manage] (measure) {Lichtmessung durchführen};
\node[usecase, below=of measure] (history) {Messhistorie auswerten};
\node[usecase, right=of measure] (advice) {Pflegehinweise erhalten};
\node[actor, below=of history] (sensor) {Smart-Sensor\\(Lichtmessung)};

\draw[-{Latex[length=3mm]}] (gardener.east) -- (manage.west);
\draw[-{Latex[length=3mm]}] (gardener.east) -- (measure.west);
\draw[-{Latex[length=3mm]}] (gardener.east) -- (history.west);
\draw[-{Latex[length=3mm]}] (gardener.east) -- (advice.west);
\draw[-{Latex[length=3mm]}] (sensor.east) -- (measure.west);
\draw[-{Latex[length=3mm]}] (sensor.east) -- (history.west);

\end{tikzpicture}
\caption{Use-Case-Diagramm der Kernabläufe}
\label{fig:usecase-diagramm}
\end{figure}

Die in Abbildung~\ref{fig:usecase-diagramm} modellierten Abläufe verdeutlichen, dass der \texttt{PlantListPresenter} und der \texttt{LightMeasurementPresenter} als zentrale Koordinatoren zwischen UI und Fachlogik agieren. Während Erstgenannter CRUD-Operationen auf \texttt{Plant}-Entitäten orchestriert, vermittelt der Lichtmess-Use-Case Messanfragen an \texttt{MeasurementRepository} und \texttt{NaturalLightRepository}, sodass Sensorwerte kontextualisiert und direkt in Pflegestrategien überführt werden können.

Die Paketstruktur in Abbildung~\ref{fig:package-diagramm} verdeutlicht die Trennung von UI-Schichten, Domänenlogik und Datenhaltung. Abhängigkeiten führen ausschließlich von oben nach unten, sodass UI-Komponenten Presenter und Repositorys nutzen, während Sensor-Adapter Messdaten kapseln.
...
\begin{tikzpicture}[
    font=\small,
    node distance=1.2cm,
    package/.style={rectangle, draw=black, thick, rounded corners, fill=orange!15, minimum width=5.6cm, minimum height=2.4cm, align=left, inner sep=6pt}
]

\node[package] (ui) {\textbf{ui}
\begin{tabular}{@{}l@{}}
- fragments\\
- activities\\
- viewmodels
...
\node[package, below=of ui] (domain) {\textbf{domain}
\begin{tabular}{@{}l@{}}
- usecases\\
- presenters\\
- lightmath
...
\node[package, below=of domain] (data) {\textbf{data}
\begin{tabular}{@{}l@{}}
- repositories\\
- dao / database\\
- entities
...
\node[package, right=3.6cm of domain] (sensors) {\textbf{sensors}
\begin{tabular}{@{}l@{}}
- camerax adapters\\
- lux converter\\
- calibration sources
...
\draw[-{Latex[length=3mm]}] (ui.south) -- (domain.north);
\draw[-{Latex[length=3mm]}] (domain.south) -- (data.north);
\draw[-{Latex[length=3mm]}] (domain.east) -- (sensors.west);
\draw[-{Latex[length=3mm]}] (ui.east) -- (sensors.north);

\end{tikzpicture}
\caption{Package-Diagramm der App-Architektur}
\label{fig:package-diagramm}
\end{figure}

Das Paketdiagramm zeigt, wie die Presenter-Schicht strikt oberhalb der Domänen- und Datenspeicherpakete positioniert ist. Diese Schichtung ermöglicht es etwa dem \texttt{PlantDetailPresenter}, Berechnungen aus \texttt{LightMath} und Zugriffsfunktionen der \texttt{PlantRepository}-Fassade zu kombinieren, während Sensoradapter wie \texttt{LightSensorHelper} isoliert angebunden bleiben und damit testbare Schnittstellen liefern.
...
Die Datenbankstruktur betont die Rolle der \texttt{Plant}-Entität als Aggregatwurzel, an die Messungen, Erinnerungen und Beleuchtungsprofile gebunden sind. Kardinalitäten wie die 1:n-Beziehung zwischen \texttt{Plant} und \texttt{Measurement} sichern Langzeitreihen, während die Kopplung von \texttt{PlantCalibration} und \texttt{NaturalLightEstimate} besondere Anforderungen der Lichtanalyse adressiert und somit präzise Empfehlungen für die Presenter-Schicht ermöglicht.
...
Die MVP-Architektur hebt hervor, dass Views ausschließlich über Presenter wie \texttt{PlantListPresenter} oder \texttt{DiaryPresenter} mit den Repositories interagieren. Diese Entkopplung stellt sicher, dass Persistenzoperationen der \texttt{PlantRepository}- und \texttt{MeasurementRepository}-Schichten konsistent ausgeführt werden, während Validierungslogik und Statusverwaltung im Presenter verbleiben.
...
\begin{thebibliography}{99}
    ...
    \bibitem{McCree1972} K. J. McCree, ``Test of the validity of the quantum yield of photosynthesis,'' \emph{Plant Physiology}, vol.~48, no.~6, pp.~703--709, 1972.
    \bibitem{NelsonBugbee2015} P. V. Nelson und B. Bugbee, ``Analysis of environmental effects on plant growth,'' \emph{Acta Horticulturae}, vol.~1099, pp.~1--12, 2015.
    \bibitem{FaustLogan2018} J. E. Faust und B. N. Logan, ``Daily light integral: A research review and high-resolution maps,'' \emph{Horticulture Research}, vol.~5, no.~4, 2018.
    \bibitem{Petersen2021} M. Petersen, ``Leveraging smartphone sensors for horticultural light assessments,'' in \emph{Proceedings of the International Symposium on Lighting in Horticulture}, 2021.
    \bibitem{DLIResearchGate} S. S. Bairagi, ``Determination of optimal daily light integral (DLI) for indoor cultivation of iceberg lettuce,'' \emph{ResearchGate}, 2023. Verfügbar unter: \url{https://www.researchgate.net/publication/372137401_Determination_of_optimal_daily_light_integral_DLI_for_indoor_cultivation_of_iceberg_lettuce_in_an_indigenous_vertical_hydroponic_system}
    \bibitem{ContinuousLighting2021} Y. Li, J. Kubota und C. Chun, ``Continuous lighting and high daily light integral enhance yield and quality of lettuce,'' \emph{Frontiers in Plant Science}, vol.~12, 2021. Verfügbar unter: \url{https://pmc.ncbi.nlm.nih.gov/articles/PMC8231634/}
    \bibitem{FrontiersTomato2023} L. Ma, Y. Zhuang, J. Wang, H. Kuang und C. Chen, ``Photosynthetic photon flux density affects fruit biomass radiation-use efficiency in tomato,'' \emph{Frontiers in Plant Science}, vol.~14, 2023. Verfügbar unter: \url{https://www.frontiersin.org/articles/10.3389/fpls.2023.1076423/full}
    \bibitem{MDPIPPFD2019} S. Kotani, T. Yamada und T. Takayama, ``Estimation of global and diffuse photosynthetic photon flux density under all sky conditions,'' \emph{Remote Sensing}, vol.~11, no.~8, 2019. Verfügbar unter: \url{https://www.mdpi.com/2072-4292/11/8/932}
    \bibitem{WikipediaDLI} Wikipedia, ``Daily light integral,'' 2024. Verfügbar unter: \url{https://en.wikipedia.org/wiki/Daily_light_integral}
    \bibitem{HLGPPFD} Horticulture Lighting Group, ``Converting PPFD to DLI,'' 2022. Verfügbar unter: \url{https://horticulturelightinggroup.ca/blogs/calculators/converting-ppfd-to-dli}

    % --- Android & Libraries ---
    \bibitem{AndroidDocs} Google LLC, ``Android developers documentation,'' 2024. Verfügbar unter: \url{https://developer.android.com/docs}
    \bibitem{AndroidStudio} Google LLC, ``Android Studio,'' 2024. Verfügbar unter: \url{https://developer.android.com/studio}
    \bibitem{GradleDocs} Gradle Inc., ``Gradle build tool user guide,'' 2024. Verfügbar unter: \url{https://gradle.org/docs/}
    \bibitem{AndroidRoom} Google LLC, ``Save data in a local database using Room,'' 2024. Verfügbar unter: \url{https://developer.android.com/training/data-storage/room}
    \bibitem{WorkManager} Google LLC, ``WorkManager overview,'' 2024. Verfügbar unter: \url{https://developer.android.com/topic/libraries/architecture/workmanager}
    \bibitem{CameraX} Google LLC, ``CameraX overview,'' 2024. Verfügbar unter: \url{https://developer.android.com/training/camerax}
    \bibitem{MaterialDesign} Google LLC, ``Material Design 3 guidelines,'' 2024. Verfügbar unter: \url{https://m3.material.io/}
    \bibitem{AndroidXReleases} Google LLC, ``AndroidX release notes,'' 2024. Verfügbar unter: \url{https://developer.android.com/jetpack/androidx/releases}
    \bibitem{FirebaseDocs} Google LLC, ``Firebase documentation,'' 2024. Verfügbar unter: \url{https://firebase.google.com/docs}
    
    % --- Beispielprojekte & Repos ---
    \bibitem{RoomCodelab} Google LLC, ``Room with a view codelab,'' 2024. Verfügbar unter: \url{https://github.com/android/codelab-android-room-with-a-view}
    \bibitem{AndroidCrudRoomMVP} AlvarezTech, ``Android CRUD Room MVP,'' GitHub Repository, 2021. Verfügbar unter: \url{https://github.com/alvareztech/android-crud-room-mvp}
    \bibitem{AndroidMVPCleanArchitecture} Smarteist, ``Android MVP clean architecture,'' GitHub Repository, 2020. Verfügbar unter: \url{https://github.com/smarteist/Android-MVP-Clean-Architecture}
    \bibitem{MVPAndroidTemplate} G. Kumar, ``MVP Android template,'' GitHub Repository, 2019. Verfügbar unter: \url{https://github.com/gauravk95/mvp-android-template}
\end{thebibliography}
